  ...
  render () {
    ...
    <div id=anchor>
      ...
      <mwc-menu quick fixed @click=${(e)=>{this.menu.show()}}>
        ...
        <mwc-list-item graphic="icon" @click=${()=>{window.app.collectionsSelector.show('kanji', getRowFromId('kanji', this.item.rowId))}}>
          <span>Add to collection</span>
          <mwc-icon slot=graphic>playlist_add</mwc-icon>
        </mwc-list-item>
        ` : nothing}
        ${this.item.type == 'words' ? html`
        <mwc-list-item graphic="icon" @click=${()=>{window.app.collectionsSelector.show('words', getRowFromId('words', this.item.rowId))}}>
          <span>Add to collection</span>
          <mwc-icon slot=graphic>playlist_add</mwc-icon>
        </mwc-list-item>
        ` : nothing}
